...
docker run -cidfile built_cid -i -v $(readlink -m ../../test_repos/rails):/tmp/repo:ro smarterclayton/centos-ruby2 /opt/openshift/prepare /tmp/repo

# Save the prepared cartridge as a deployment artifact (image representing runtime)
cid=$(cat built_cid)
docker commit -run='{"WorkingDir": "/opt/openshift/cartridge", "Cmd": ["bundle", "exec", "rackup"], "PortSpecs": ["9292"]}' $cid smarterclayton/centos-ruby2-code
